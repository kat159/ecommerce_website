 ...
 *  add/update/delete: cached change
 *  submit: save all changes into database & firebase storage
 */
class CrudService {
    constructor(serviceKey, service, serviceLayer) {
    ...
    page = async (props) => {
        if (this.service.page) {
            return (await this.service.page(props)).data
        }
    }
    getAllOption = async (props) => {
        if (this.service.getAllOption) {
            return (await this.service.getAllOption(props)).data
        }
    }
    add = async (dto, parentId, childId) => { // cached add, not to database
        ...
        this.addCache.set(dto.id,
            {
                dto: dto, parentId: parentId, childId: childId,
                service: this,
                layer: this.serviceLayer
            }
        )
    ...
    update = async (dto) => {  // cached update, not to database
        if (!dto.id) {
            ...
            this.updateCache.set(dto.id, { dto, service: this })
    ...
    remove = async (id) => { // cached remove, not to database
        ...
        if (this.addCache.has(id)) {
            ...
            this.removeCache.set(id, { id, service: this })
    ...
    get = async (id) => {
        // **search bottom up, the closer the layer, the latest the change
        ...
        if (updateCacheContainingId) {
            return updateCacheContainingId.get(id).dto
        }
    ...
    _originalDataFilter = (data) => {
        return [...data.filter(item => !this._bottomUpSearchRemoveCacheContainingId(item.id)) // dont show cached removed record
            .map(item => {
                const updateCacheContainingId = this._bottomUpSearchUpdateCacheContainingId(item.id)
                if (updateCacheContainingId) {
                    return updateCacheContainingId.get(item.id).dto
                }
                return item
            })]
    }
    ...
    _addToDataBase = async ({ dto, childId, parentId }) => {
        await this._saveFiles(dto)
        const { id: tmpId, ...rest } = dto
        let res;
        if (parentId) {
            if (childId) // many to many join table
                res = await this.service.addAll({ id: parentId, cid: childId }, [rest])
            else {// one to many
                res = await this.service.addAll({ id: parentId }, [rest])
            }
        } else {
            res = await this.service.addAll([rest])
        }

        const realId = res.data[0]
        return realId
    }
    _updateToDatabase = async (dto) => { // update/delete 都是original data， 不需要提供parentId，独立于addAllToDatabase
        const oldDto = (await this.service.get({ id: dto.id })).data
        await this._saveFiles(dto, oldDto)
        await this.service.updateAll([dto])
    }
    _removeToDatabase = async (id) => { // update/delete 都是original data， 不需要提供parentId，独立于addAllToDatabase
        const oldDto = (await this.service.get({ id })).data


        await this._saveFiles(null, oldDto)
        await this.service.removeAll([id])
    }
    _addAllToDatabase = async () => {
        // add/getAll 需要提供parentId， 用于给子表提供外键, 不需要childId， 取出所有join table的addCache， childId存在于addCache中
        const addFailedList = []

        const tmpPidToReadPidMap = new Map()
        const addList = [...this.addCache.values()]

        const layerToAddListMap = new Map()
        addList.forEach((props) => {
            const { layer } = props
            if (!layerToAddListMap.has(layer)) {
                layerToAddListMap.set(layer, [])
            }
            layerToAddListMap.get(layer).push(props)
        })
        const layers = [...layerToAddListMap.keys()].sort((a, b) => a - b)

        /** the add request in the next layer should wait until all request in this layer complete */
        for (const layer of layers) {
            const addList = layerToAddListMap.get(layer)
            const promises = []
            addList.forEach((props) => { // 别用for of， 应该异步处理每个dto
                let { dto, childId, parentId, service } = props
                if (tmpPidToReadPidMap.has(parentId)) {
                    parentId = tmpPidToReadPidMap.get(parentId)
                }
                const { id: tmpId, ...rest } = dto
                const p = service._addToDataBase({ dto: rest, childId, parentId }).then((realId) => {
                    tmpPidToReadPidMap.set(tmpId, realId)
                }).catch((err) => {
                    addFailedList.push({ dto, childId, parentId })
    ...
    _updateAllToDatabase = async () => { // update/delete 都是original data， 不需要提供parentId，独立于addAllToDatabase
        ...
        this.updateCache.forEach(({ dto, service }) => {
            const p = service._updateToDatabase(dto).catch(e => {
                console.error('update failed', e)
                updateFailedList.push({ dto })
            })
            promises.push(p)
        })
        ...
        // for (const child of this.children) { // 不需要， 已经全部merge到这里了
    ...
    _removeAllToDatabase = async () => { // update/delete 都是original data， 不需要提供parentId，独立于addAllToDatabase
        const promises = []
        this.removeCache.forEach(({ id, service }) => {
            const p = service._removeToDatabase(id).catch(e => {
    ...
    submit = async () => { // submit change to database
        ...
        const {
            globalAddCache, globalUpdateCache, globalRemoveCache
        } = this._getGlobalCaches()
...
class CrudManager {
    ...
    register = (displayerProps, parent, layer = 0) => {
        const serviceKey = uuid()
        const service = displayerProps.service
        const crudService = new CrudService(serviceKey, service, layer)
        if (layer === 0) this.rootService = crudService
            ...
            editableFields.forEach(field => {
                if (field.displayerProps) { // relational data
                    crudService.children.push(
                        this.register(field.displayerProps, crudService, layer + 1)
                    )
